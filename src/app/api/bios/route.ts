...
import { db } from "@/db";
import { bios } from "@/db/schema";
import { eq, desc } from "drizzle-orm";

export async function GET() {
  try {
    const rows = await db.select().from(bios).orderBy(desc(bios.createdAt));
...
export async function POST(req: NextRequest) {
  try {
    ...
    const [created] = await db.insert(bios).values(body).returning();
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await db.delete(bios).where(eq(bios.id, parseInt(id)));
